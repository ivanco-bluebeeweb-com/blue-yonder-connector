# PST Testing Scenarios for Blue Yonder Connector

## Part A: Authentication & Connectivity Verification
- **Scenario A1: Valid Credential Connect**: `connect_blue_yonder_connector` saves token and validates endpoint.
...
- **Scenario B1: List Shipments**: `list_shipments` returns typed list with bounds.
- **Scenario B2: Get Shipment**: `get_shipment` returns entity details or 404 error.
- **Scenario B3: Health Audit**: `audit_shipment_health` aggregates active records.

## Part C: Safe Write & Idempotency Testing
- **Scenario C1: Connection Lifecycle**: Clean disconnect via `disconnect_blue_yonder_connector` without lingering secrets.
