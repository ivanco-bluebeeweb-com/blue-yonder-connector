"""Connection lifecycle for Blue Yonder Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from blue_yonder_client import BlueYonderClient
...
_SECRET = "blue_yonder_connections"
...
@chat.function(
    "connect_blue_yonder",
    "Connect Blue Yonder account via credentials.",
    action_type="write",
    chain_callable=True,
    event="blue-yonder-connector.connect_blue_yonder",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_blue_yonder(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Blue Yonder Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Blue Yonder Account",
...
@chat.function(
    "list_connections",
    "List connected Blue Yonder accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_blue_yonder",
    "Disconnect Blue Yonder account.",
    action_type="write",
    chain_callable=True,
    event="blue-yonder-connector.disconnect_blue_yonder",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_blue_yonder(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
